...
_∷'_ : {a : Level}{A : Set a} → A → List A → List A
a ∷' l = a ∷ l
...
lemma++[] : { A : Set}(l : List A) → l ++ [] ≡ l
lemma++[] {A} [] = refl
lemma++[] {A} (x ∷ l) = cong (λ l' → x ∷ l') (lemma++[] l)
...
lemmaListAssoc (x ∷ l1) l2 l3 = cong (λ l → x ∷ l) (lemmaListAssoc l1 l2 l3)
...
lemmaListAssoc4 l1 l2 l3 l4 =
                 (l1 ++ (l2 ++ (l3 ++ l4)))
                      ≡⟨ cong (λ l → l1 ++ l) (lemmaListAssoc l2 l3 l4)  ⟩
                 (l1 ++ ((l2 ++ l3) ++ l4))
                      ≡⟨ lemmaListAssoc l1 (l2 ++ l3) l4  ⟩
                 ((l1 ++ (l2 ++ l3)) ++ l4)
                      ≡⟨ cong (λ l → l ++ l4) (lemmaListAssoc l1 l2 l3)  ⟩
                (((l1 ++ l2) ++ l3) ++ l4)
                     ∎
